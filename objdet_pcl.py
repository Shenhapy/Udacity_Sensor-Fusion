...
def show_pcl(pcl):
    ...
    def key_callback(vis):
        vis.close()
    # Create a window with the key callback function 
...
def show_range_image(frame, lidar_name):
    ...
    img_centerr = int(img_range.shape[1]/2)
    img_range = img_range[:,img_centerr-deg90:img_centerr+deg90]  
    ...
    img_center = int(img_intensity.shape[1]/2)
    img_intensity = img_intensity[:,img_center-deg90:img_center+deg90] 
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 0] = np.int_(lidar_pcl_cpy[:, 0] / bev_discret)
    ...
    lidar_pcl_cpy= lidar_pcl_cpy[np.lexsort((-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))]
    ...
    intensity_map[lidar_pcl_top[:, 0].astype(int), lidar_pcl_top[:, 1].astype(int)] = (lidar_pcl_top[:, 3]) / (np.amax(lidar_pcl_top[:, 3]) - np.amin(lidar_pcl_top[:, 3]))
    ...
    upper_height = configs.lim_z[1]
    lower_height = configs.lim_z[0]
    # normalized_height = (lidar_pcl_top[:, 2] - lower_height) / (upper_height - lower_height)
    normalized_height = (lidar_pcl_top[:, 2]) / (upper_height - lower_height)
    _, indx_height = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_index=True)
    lidar_height = lidar_pcl_top[indx_height]
